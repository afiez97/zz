...
$("#no_kad_pengenalan").on("change", function(){
    ...
    form.append("no_kad_pengenalan", $("#no_kad_pengenalan").val());
    checkLoginAdmin(form, function(){
        if(objCheck.success){
            $("#FK_capaian").removeClass('hidden');
            $("#FK_capaian").html('<option value="">Pilih Peranan</option>');
            let dataCheck = objCheck.data;
            
            $.each(dataCheck,function(i,field){
                $("#FK_capaian").append('<option value="'+field.id_peranan+'">'+field.nama_peranan+'</option>');
            });
        } else  {
    ...
    if (!confirmed) {
        ...
        request.done(function (response) {
            ...
            if (!result.success) {
                ...
                var FK_capaian = $("#FK_capaian").val();
                sessionStorage.capaian = capaian[(parseInt(FK_capaian) - 1)];
                switch(FK_capaian) {
                    case '1':   window.location.replace('../super/'); break;
                    case '2':   window.location.replace('../ptl/'); break;
                    case '3':   window.location.replace('../ppp/'); break;
                    case '4':   window.location.replace('../us/'); break;
                }
            }
        });
